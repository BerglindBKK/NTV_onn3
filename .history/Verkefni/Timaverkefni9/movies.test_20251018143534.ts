...
describe("movies.ts - Data Logic Unit Tests", () => {
  ...
  describe("loadMoviesPaginatedAsync", () => {
    ...
    i("should throw an error for malformed JSON", async () => {
      //búum til error og látum mocked taka hann inn
      const writeError = new Error("Write Error failure");
      vi.mocked(writeFile).mockRejectedValue(writeError);
      await expect(
        loadMoviesPaginatedAsync("dummy/json", undefined, undefined)
      ).rejects.toThrow("loadMoviesPaginatedAsync: writeFile failure");
    });
  });
