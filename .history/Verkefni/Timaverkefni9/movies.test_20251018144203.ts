...
describe("movies.ts - Data Logic Unit Tests", () => {
  ...
  describe("loadMoviesPaginatedAsync", () => {
    ...
    it.todo("should throw an error if readFile fails", async () => {
      it("should throw an error if readFile fails", async () => { búum til error og látum mocked taka hann inn const writeError = new Error("Write Error failure"); vi.mocked(writeFile).mockRejectedValue(writeError); await expect( loadMoviesPaginatedAsync("dummy/json", undefined, undefined) ).rejects.toThrow("loadMoviesPaginatedAsync: writeFile failure"); });
    });
