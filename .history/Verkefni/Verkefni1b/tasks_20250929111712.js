...
export function listTasks() {
    const tasks = loadTasks(); 
    if (!tasks)  console.log(chalk.yellow('No tasks found. Add a task to get started!'));
    if (!tasks)  console.log(chalk.blue('Your Tasks:'));
    tasks.forEach((task, index) => {
    const checkmark = task.done ? "[✔]" : "[ ]";   // pick symbol
