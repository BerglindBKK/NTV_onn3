...
ALTER TABLE
  albums
ADD
  COLUMN is_explicit BOOLEAN NOT NULL DEFAULT FALSE;
...
SELECT
  artists.name AS artist_name,
  count(album_artists.album_id) AS total_albums
FROM
  artists
...
-- left join to include album with no songs
-- ============================================
...
--STRING_AGG to combine artist and genre names
...
SELECT
  ...
  songs.duration_seconds AS song_duration
FROM
  songs
...
SELECT
  artists.name,
  count(DISTINCT genres.id) AS total_genres
FROM
  artists
  JOIN album_artists ON artists.id = album_artists.artist_id
...
SELECT
  ...
  count(albums.id) AS albums_cound
FROM
  albums
  JOIN album_genres ON album_genres.album_id = albums.id
...
SELECT
  ...
  round(avg(albums.rating), 2) AS album_rating
FROM
  artists
...
SELECT
  artists.name,
  sum(albums.sales_millions) as sales_sum,
...
-- Sort by most recent decade first
SELECT
  (albums.release_year / 10) * 10 as decade,
  count(albums.id) as album_count,
  round(avg(albums.rating), 2) as avg_rating,
  round(sum(albums.sales_millions), 2) AS total_sales_mill
FROM
  albums
GROUP BY
  decade
...
GROUP BY
  ...
  artists.name
HAVING
  COUNT(DISTINCT songs.id) >= 10
ORDER BY
  albums.rating DESC;

--HAVING since comparing aggregated songs count
...
SELECT
  ...
  ROUND(AVG(reviews.rating), 2) as avg_rev_rating
FROM
  albums
  JOIN reviews ON reviews.album_id = albums.id
GROUP BY
...
FROM
  ...
  JOIN genres ON genres.id = album_genres.genre_id
WHERE
  albums.release_year BETWEEN 2010
  AND 2019
GROUP BY
  genres.name
ORDER BY
...
-- LEFT JOIN since album should still return without reviews or songs
...
(
  SELECT
    'album' AS type,
    a.title AS name,
    CONCAT('Year: ', a.release_year, 'Rating:', a.rating) AS context
  FROM
    albums a
  WHERE
    a.title ILIKE '%love%'
...
ALL (
  SELECT
    'artist' AS type,
    ar.name AS name,
    CONCAT('Country: ', COALESCE(ar.country, '-')) AS context
  FROM
    artists ar
  WHERE
    ar.name ILIKE '%love%'
...
ALL (
  SELECT
    'songs' AS type,
    s.title as name,
    CONCAT('Album: ', COALESCE(a.title, '-')) AS context
  FROM
    songs s
