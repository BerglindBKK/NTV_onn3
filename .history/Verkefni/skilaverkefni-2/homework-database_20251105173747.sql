...
SELECT albums.title, round(avg(reviews.rating), 2) AS average_rating, count(reviews.id)
FROM albums
...
SELECT artists.name, sum(albums.sales_millions) as sales_sum, count(albums.id) as total_albums
FROM artists
JOIN album_artists ON album_artists.artist_id = artists.id
JOIN albums ON album_artists.album_id = albums.id
GROUP BY artists.name
...
(
SELECT
'album' AS type, a.title AS name, 
CONCAT('Year: ', a.release_year, 'Rating:', a.rating) AS context
FROM albums a
WHERE a.title ILIKE '%love%'
)
UNION ALL
(
SELECT
'artist' AS type, ar.name AS name,
CONCAT('Country: ', COALESCE(ar.country, '-')) AS context
FROM artists ar
WHERE ar.name ILIKE '%love%'
)
UNION ALL
(SELECT 'songs' AS type, s.title as name, 
CONCAT('Album: ', COALESCE(a.title, '-')) AS context
FROM songs s
LEFT JOIN albums a ON a.id = s.album_id
WHERE s.title ILIKE '%love%'
)
ORDER BY type, name;
...
-- Sort by highest average rating first
SELECT g.name AS genre_name, ROUND(AVG(a.rating), 2) AS avg_rating, COUNT(a.id) AS album_count
FROM genres g
JOIN album_genres ag ON ag.genre_id = g.id
JOIN albums a ON a.id = ag.album_id
GROUP BY g.name
HAVING AVG(a.rating) > 7.5 AND COUNT(a.id) >=3
